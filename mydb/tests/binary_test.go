...
func TestBinaryOperations(t *testing.T) {
	...
	if selectQuery.Table != "users" {
		t.Errorf("Table attendue: users, obtenue: %s", selectQuery.Table)
	}

	// Tester le parsing d'une requête INSERT
	insertQuery := "INSERT INTO users (id, name) VALUES (1, 'John')"
	insert, err := parser.ParseInsert(insertQuery)
	if err != nil {
		t.Errorf("Erreur de parsing INSERT: %v", err)
	}
	if insert.Table != "users" {
		t.Errorf("Table attendue: users, obtenue: %s", insert.Table)
	}
}
